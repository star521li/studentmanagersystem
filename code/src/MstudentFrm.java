...
public class MstudentFrm implements ActionListener {
	String cksql="select students.* from students where 1=1";
	JFrame w;
	JDialog jdl=new JDialog(w,"学生信息管理",true);
	//显示信息的表
	String[] stafflistcols={"id","学号","姓名","性别","班级"};
	//设置表格整行显示
	...
	JLabel mggenderlb=new JLabel("性别：");
	@SuppressWarnings("rawtypes")
	JComboBox mggendertxt=new JComboBox();
	JLabel mgclsslb=new JLabel("班级：");
	...
	MstudentFrm(JFrame jf){
		...
		jpl.add(mggenderlb);
		mggenderlb.setBounds(360, 30, 50, 20);
		jpl.add(mggendertxt);
		mggendertxt.setBounds(420, 30, 60, 20);
		jpl.add(mgclsslb);
		...
		mggendertxt.addItem("男");
		mggendertxt.addItem("女");
		...
		stafflist.addMouseListener(new java.awt.event.MouseAdapter(){
            public void mouseClicked(MouseEvent e) {
              ...
              mggendertxt.setSelectedItem(stafflist.getValueAt(r, 3).toString());
              mgclsstxt.setText(stafflist.getValueAt(r, 4).toString());
            }
        });
		//为增删改重置功能添加监听事件
	...
	class AddAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			String gender=mggendertxt.getSelectedItem().toString().trim();
			String clssname=mgclsstxt.getText().toString().trim();
			if("".equals(sno)||"".equals(sname)||"".equals(gender)||"".equals(clssname)){
			...
			int x=db.executeUpdate("insert into students(sno,sname,gender,clssname) values('"+sno+"','"+sname+"','"+gender+"','"+clssname+"')");
			if(x>0){
	...
	class DelAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			DbHelper db=new DbHelper();
			if("".equals(id)||!db.checkTrue("select id from students where id="+id)){
				JOptionPane.showMessageDialog(null,"请选择所要删除的信息!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}			
			int x=db.executeUpdate("delete from students where id="+id);
			if(x>0){
	...
	class UpdAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			DbHelper db=new DbHelper();
			rs=db.executeQuery("select id from students where clssname='"+sno+"'");
			try {
				while(rs.next()){
			...
			int x=db.executeUpdate("update students set sno='"+sno+"',sname='"+sname+"',gender='"+gender+"',clssname='"+clssname+"' where id="+id);
			if(x>0){
	...
	public void getData(String sql){
		...
		if(rs!=null){
			try {
				while(rs.next()){
					model.addRow(new String[]{String.valueOf(rs.getInt("ID")),rs.getString("sno"),rs.getString("sname"),rs.getString("gender"),rs.getString("clssname")});
				}
			} catch (SQLException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
