...
public class McourseFrm implements ActionListener {
	String cksql="select * from course";
	JFrame w;
	JDialog jdl=new JDialog(w,"课程信息管理",true);
	//显示信息的表
	String[] stafflistcols={"id","课程名称"};
	//设置表格整行显示
	...
	McourseFrm(JFrame jf){
		...
		//数据管理部分布局
		jpl.add(mgsnolb);
		mgsnolb.setBounds(50, 30, 80, 20);
		jpl.add(mgsnotxt);
		mgsnotxt.setBounds(140, 30, 150, 20);
		jpl.add(addbt);
	...
	class AddAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if("".equals(sno)){
				JOptionPane.showMessageDialog(null,"课程信息不完整!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}
			DbHelper db=new DbHelper();
			if(db.checkTrue("select id from course where cname='"+sno+"'")){
				JOptionPane.showMessageDialog(null,"课程名称已存在!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}
			int x=db.executeUpdate("insert into course(cname) values('"+sno+"')");
			if(x>0){
	...
	class DelAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			DbHelper db=new DbHelper();
			if("".equals(id)||!db.checkTrue("select id from course where id="+id)){
				JOptionPane.showMessageDialog(null,"请选择所要删除的信息!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}			
			int x=db.executeUpdate("delete from course where id="+id);
			if(x>0){
	...
	class UpdAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			DbHelper db=new DbHelper();
			rs=db.executeQuery("select id from course where cname='"+sno+"'");
			try {
				while(rs.next()){
			...
			int x=db.executeUpdate("update course set cname='"+sno+"' where id="+id);
			if(x>0){
	...
	public void getData(String sql){
		...
		if(!"".equals(no.getText())){
			sql+=" where cname like '%"+no.getText().trim()+"%'";
		}
		ResultSet rs=db.executeQuery(sql);
		if(rs!=null){
			try {
				while(rs.next()){
					model.addRow(new String[]{String.valueOf(rs.getInt("ID")),rs.getString("cname")});
				}
			} catch (SQLException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
