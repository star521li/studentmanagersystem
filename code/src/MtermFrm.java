...
public class MtermFrm implements ActionListener {
	String cksql="select * from terms";
	JFrame w;
	JDialog jdl=new JDialog(w,"学期信息管理",true);
	//显示信息的表
	String[] stafflistcols={"id","学期名称"};
	//设置表格整行显示
	...
	class AddAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if("".equals(sno)){
				JOptionPane.showMessageDialog(null,"学期信息不完整!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}
			DbHelper db=new DbHelper();
			if(db.checkTrue("select id from terms where tmname='"+sno+"'")){
				JOptionPane.showMessageDialog(null,"学期名称已存在!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}
			int x=db.executeUpdate("insert into terms(tmname) values('"+sno+"')");
			if(x>0){
	...
	class DelAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			DbHelper db=new DbHelper();
			if("".equals(id)||!db.checkTrue("select id from terms where id="+id)){
				JOptionPane.showMessageDialog(null,"请选择所要删除的信息!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}			
			int x=db.executeUpdate("delete from terms where id="+id);
			if(x>0){
	...
	class UpdAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			DbHelper db=new DbHelper();
			rs=db.executeQuery("select id from terms where tmname='"+sno+"'");
			try {
				while(rs.next()){
			...
			int x=db.executeUpdate("update terms set tmname='"+sno+"' where id="+id);
			if(x>0){
	...
	public void getData(String sql){
		...
		if(!"".equals(no.getText())){
			sql+=" where tmname like '%"+no.getText().trim()+"%'";
		}
		ResultSet rs=db.executeQuery(sql);
		if(rs!=null){
			try {
				while(rs.next()){
					model.addRow(new String[]{String.valueOf(rs.getInt("ID")),rs.getString("tmname")});
				}
			} catch (SQLException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
